...
try: inputFileNames = sys.argv[1]
    ...
    inputFileNames = "/eos/uscms//store/user/lpcsusyhad/sbein/cmsdas19/Ntuples/Summer16.DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1_104_RA2AnalysisTree.root"
...
for fname in inputFiles:
    name2avoidPunishment = fname.replace('/eos/uscms/','root://cmsxrootd.fnal.gov//')
    print 'adding file:', name2avoidPunishment    
    c.Add(fname)
    break
...
x_ = len(inputFiles)
print 'will analyze file:', inputFileNames


####a bunch of arguments that should probably be made into command line args
if 'Run20' in inputFileNames: isdata = True
else: isdata = False
if 'Run2016' in inputFileNames or 'Summer16' in inputFileNames: phase_ = 0
else: phase_ = 1
...
####some stuff for pile-up dependencies studies
...
####dtmode specifies whether to consider long or short tracks
if dtmode == 'PixOnly': 
	PixMode = True
	PixStripsMode = False
	CombineMode = False
...
####global variable set in this nice location:
lepPtCut = 20
...
if 'Run2016' in inputFileNames: hMask = fMask.Get('hEtaVsPhiDT_maskData-2016Data-2016')
	...
	fMask.Get('hEtaVsPhiDT_maskMC-2016MC-2016')
	hMask = '' ########################################################note the mask is shut off?
if MakeMask: hMask = ''
#=====This sets up the smearing
dResponseHist_el = {}
dResponseHist_mu = {}
...
#=====This sets up histograms for the invariant mass and kappa histograms    
dInvMassElRECOHist = {}
dInvMassElDTHist = {}
hElProbePt_DTnums = {}
hElProbePt_RECOdens = {}
...
for ientry in range(nentries):
    ...
    if not MakeMask:
    	if not c.BTags==0: continue
    	if not adjustedNJets<4: continue
